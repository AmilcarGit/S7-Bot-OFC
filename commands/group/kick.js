export async function ejecutar(sock, info, args, contexto) {
  ...
  if (!mencionado) {
    return sock.sendMessage(contexto.chatId, { text: "⚠️ Menciona al usuario que quieres expulsar." }, { quoted: info });
  }
  try {
    await sock.groupParticipantsUpdate(contexto.chatId, [mencionado], "remove");
    await sock.sendMessage(contexto.chatId, { text: "✅ Usuario expulsado correctamente." });
  } catch (err) {
    await sock.sendMessage(contexto.chatId, { text: "❌ No pude expulsar (¿soy administrador del grupo?)." });
  }
}