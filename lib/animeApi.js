...
// Convierte un buffer GIF a un buffer MP4 real usando ffmpeg del sistema
async function gifBufferToMp4(gifBuffer) {
  ...
  await new Promise((resolve, reject) => {
    ...
    ff.on("error", reject); // ffmpeg no instalado, etc.
    ff.on("close", (code) => {
      if (code === 0) resolve();
      else reject(new Error("ffmpeg falló: " + stderr.slice(-300)));
    });
  });

  const mp4Buffer = await fs.readFile(outPath);

  // Limpieza de temporales (no bloqueante)
  ...
  try {
    ...
    if (isGif) {
      try {
        ...
        // Si ffmpeg no está instalado o falla, mandamos el gif como documento
        // para que al menos se pueda ver/descargar en vez de romperse.
        ...
        await sock.sendMessage(
          contexto.chatId,
          {
            ...
            mimetype: "image/gif",
            fileName: `${tipo}.gif`,
            caption: caption + "\n┃ ⚠️ No se pudo animar (falta ffmpeg en el servidor)",
          },
          { quoted: info }
        );
      }
