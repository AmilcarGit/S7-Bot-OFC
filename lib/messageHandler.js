...
function extraerCandidatosId(info, esGrupo) {
  const candidatos = new Set();

  const principal = esGrupo ? info.key.participant : info.key.remoteJid;
  if (principal) candidatos.add(principal.split("@")[0]);

  // Algunos forks de Baileys exponen el JID "alterno" (número real de teléfono)
  // cuando WhatsApp está direccionando con @lid en vez del número.
...
export async function handleMessage(sock, m) {
  ...
  const idsRemitente = extraerCandidatosId(info, esGrupo);
  const contexto = {
    chatId,
    esGrupo,
    remitente,
